...
Документ описывает фактическое устройство приложения по состоянию исходного кода. Для понимания причин отдельных решений (почему MVVM, почему импорт, почему каскад резолверов) см. [scope-deviations.md](scope-deviations.md).
